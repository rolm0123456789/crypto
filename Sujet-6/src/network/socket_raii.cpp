...
namespace CryptoNetwork {
    ...
    std::optional<SocketRAII> SocketRAII::create_tcp_socket() noexcept {
        ...
        if (fd == INVALID_FD) return std::nullopt;

        // Option réutilisation d'adresse pour éviter le blocage du port en état TIME_WAIT
        int opt = 1;
    ...
    bool SocketRAII::bind_to(std::uint16_t port) noexcept {
        ...
        addr.sin_addr.s_addr = INADDR_ANY; // Écoute sur toutes les interfaces locales
    ...
    std::optional<SocketRAII> SocketRAII::accept_connection() noexcept {
        ...
        if (client_fd == INVALID_FD) return std::nullopt;
    ...
    bool SocketRAII::connect_to(const std::string& address, std::uint16_t port) noexcept {
        ...
        if (::inet_pton(AF_INET, address.c_str(), &server_addr.sin_addr) <= 0) return false;
    ...
    bool SocketRAII::send_all(std::span<const std::uint8_t> data) noexcept {
        ...
        while (bytes_left > 0) {
            ...
            if (bytes_sent <= 0) return false; // Erreur ou déconnexion
            ptr += bytes_sent;
    ...
    bool SocketRAII::recv_exact(std::span<std::uint8_t> out_buffer) noexcept {
        ...
        while (bytes_left > 0) {
            ...
            if (bytes_received <= 0) return false; // Erreur ou déconnexion fermée prématurément
            ptr += bytes_received;
